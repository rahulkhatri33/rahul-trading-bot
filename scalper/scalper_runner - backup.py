                ...
                try:
                    ...
                    try:
                        ...
                        if risk <= 0 or reward <= 0:
                            logger.log_error(f"{symbol} ❌ Skipping {side} trade: Non-positive risk/reward (check SL/TP).")
                            continue
...
def execute_trade(symbol: str, qty: float, side: str, price: float, sl: float, tp: float, trailing_stop: float):
    """Execute a trade on Binance Futures with:
       - single position per coin,
       - SL/TP calculated and stored (no SL/TP open orders),
       - SAFE REVERSAL: close old only after all preflight checks pass.
    """
    config = CONFIG
    scalper_settings = config.get("scalper_settings", {})
    symbol_precisions = scalper_settings.get("symbol_precisions", {}).get(symbol, {})
    ...
    try:
        ...
        # Try to capture a realistic entry price from Binance response
        entry_price = None
        try:
            # Binance often returns 'avgPrice' or 'fills' with price; fallback to Binance utils price
            entry_price = float(order.get('avgPrice') or order.get('fills', [{}])[0].get('price') or price)
        except Exception:
            entry_price = price
        ...
        # Partial TP configuration and logging
        ...
        if ptp_cfg.get("enabled", False):
            ...
            rr = abs(tp - entry_price) / abs(entry_price - sl) if sl != entry_price else None
            if rr:
                # TP1 at nR distance from entry
                first_tp = entry_price + (entry_price - sl) * rr_first if direction == "long" else entry_price - (sl - entry_price) * rr_first
                logger.log_info(f"{symbol} 📊 Partial TP plan | first_rr={rr_first}, first_size_pct={first_size_pct}, calc_RR={rr:.3f}, TP1={first_tp}, SL={sl}, TP={tp}")
                ...
                try:
                    ...
                    logger.log_warning(f"{symbol} ⚠️ Failed to write partial TP to position: {e}")
        ...
        # Provide clearer logs for precision / margin errors
